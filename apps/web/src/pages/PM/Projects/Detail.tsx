/**
 * 项目详情 Drawer — 全字段内联编辑 + 铁三角 + 里程碑 + 周进度 (规范 §3.5)
 */
...
export default function ProjectDetail() {
  ...
  const save = async (field: string, value: any) => {
    try { await updateProject(p.id, { [field]: value } as any); message.success('已保存'); }
        ...
        <Section title="W1-W4 周进度">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
            {[1,2,3,4].map(w => {
              ...
              const prog = p[`w${w}Progress`] || '';
              const target = p[`w${w}Target`] || '';
              return (
