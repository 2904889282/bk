...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initPermissions() {
        execute("INSERT IGNORE INTO sys_permission (id, code, name, is_deleted) VALUES " +
                // 管线
                "(1, 'pipeline:list', '管线列表', 0), (2, 'pipeline:create', '创建管线', 0), " +
                "(3, 'pipeline:edit', '编辑管线', 0), (4, 'pipeline:delete', '删除管线', 0), " +
                "(5, 'pipeline:import', '导入管线', 0), (6, 'pipeline:batch-delete', '批量删除', 0), " +
    ...
    private void ensureSeedUser(Long id, String username, String rawPassword, String realName, String roleType) {
        ...
        if (user == null) {
            user = userMapper.selectOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getUsername, username));
        }
    ...
    private void initRolePermissions() {
        ...
        // 兼容早期数据库：权限表已存在但缺少管线列表/角色绑定时，启动时自动补齐。
        execute("INSERT IGNORE INTO sys_permission (code, name, is_deleted) VALUES " +
                "('pipeline:list', '管线列表', 0), ('pipeline:create', '创建管线', 0), " +
                "('pipeline:edit', '编辑管线', 0), ('pipeline:delete', '删除管线', 0)");
        execute("INSERT IGNORE INTO sys_role_permission (role_id, permission_id) " +
                "SELECT 1, id FROM sys_permission WHERE code LIKE 'pipeline:%'");
        execute("INSERT IGNORE INTO sys_role_permission (role_id, permission_id) " +
                "SELECT 2, id FROM sys_permission WHERE code LIKE 'pipeline:%'");
    }
