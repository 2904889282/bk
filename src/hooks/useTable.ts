...
export function useTable<T extends Record<string, unknown>, P extends Record<string, unknown> = Record<string, never>>(
  ...
  const loadData = useCallback(async (pn?: number, ps?: number) => {
    ...
    try {
      const res = await fetchApi({ ...filters, pageNum, pageSize });
