...
interface Identifiable { id: number; [key: string]: unknown; }
...
export function useFormDialog<T extends Record<string, unknown>>(options: UseFormDialogOptions<T>) {
  ...
  const openCreate = useCallback(() => {
    ...
    formRef.current?.setFieldsValue(defaultValues as Record<string, unknown>);
    setOpen(true);
  }, [defaultValues]);

  const openEdit = useCallback((record: T & Identifiable) => {
    setEditingId(record.id);
    formRef.current?.setFieldsValue(record as Record<string, unknown>);
    setOpen(true);
  }, []);
