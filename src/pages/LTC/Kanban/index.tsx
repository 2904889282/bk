...
export default function LtcKanban() {
  ...
  const getColumnItems = (stages: PipelineStage[]) =>
    list.filter((p) => stages.includes(p.stage as PipelineStage));
                ...
                {items.length === 0 ? (
                  ...
                  items.map((p) => (
                    ...
                    >
                      ...
                      <Space size={4}>
                        <Tag color={STAGE_COLORS[p.stage as keyof typeof STAGE_COLORS]}>
                          {STAGE_MAP[p.stage as keyof typeof STAGE_MAP]}
                        </Tag>
                        <Progress
                          percent={getProgressValue(p.stage)}
                          size="small"
