...
export default function ProjectImportModal({ open, onClose, onImported }: Props) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data.fail > 0 || data.skip > 0) {
          ...
          message.success(`成功导入 ${data.success} 个项目`);
        }
        onImported();
      }
    } catch {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const token = localStorage.getItem('beike_token') || '';
      const res = await fetch('/api/projects/import/template', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = '项目导入模板.xlsx'; a.click();
  ...
  const handleDownloadErrors = async () => {
    ...
    try {
      const token = localStorage.getItem('beike_token') || '';
      const res = await fetch('/api/projects/import/errors', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = '项目导入错误明细.xlsx'; a.click();
      URL.revokeObjectURL(url);
    } catch {
  ...
  return (
    <Modal title="📥 导入项目" open={open} onCancel={() => { resetState(); onClose(); }}
